...
Map your security audit findings to a score from 1 to 10:
- **8 - 10**: Robust input validation, correct RBAC check implementation, safe secret management, and zero vulnerability concerns.
- **5 - 7**: Generally secure, but has minor areas of improvement like stricter CORS limits or better signature checks.
- **1 - 4**: Exploitable vulnerabilities present (Critical/High findings), credentials exposed, or direct threat to trust boundaries.
...
- **Do not invoke from another persona.** If `code-reviewer` flags something that warrants a deeper security pass.