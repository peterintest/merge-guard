...
def test_chat_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    session_url = f"{BASE_URL}/apps/merge_guard/users/{user_id}/sessions"
    ...
    # Send a valid JSON PR event payload to prevent validation error
    data = {
        ...
        "new_message": {
            "role": "user",
            "parts": [{"text": '{"repository": "google/adk", "pr_number": 101}'}],
        },
        "streaming": True,
    }
    ...
    # Verify that the final event output contains the TriageDecisionOutput dictionary
    final_event = events[-1]
    output_data = final_event.get("output")
    assert output_data is not None, "Expected final event output to not be None"
...
def test_collect_feedback(server_fixture: subprocess.Popen[str]) -> None:
    ...
    # First, create the session for feedback verification
    session_url = f"{BASE_URL}/apps/merge_guard/users/test-user-{uid}/sessions"
    session_response = requests.post(
        session_url,
        headers=HEADERS,
        json={"session_id": f"test-session-{uid}"},
        timeout=60,
    )
    assert session_response.status_code == 200
